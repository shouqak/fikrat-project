...
import { CheckCircle, CircleCheckBig, Clock, EllipsisVertical, Lightbulb, User, XCircle } from "lucide-react"
...
const handleConfirmDelete = async () => {
            ...
            <div className="flex items-center">
              ...
              <div>
                <p className="text-lg me-2 mt-3">
                  معلمك: {teacherFind[user.teacherId] || "لم يتم تعيين معلم بعد"}
                </p>
              </div>
            </div>
          </div>
            ...
            {approvedIdeas.length > 0 ? (
              approvedIdeas.map((item) => (
                ...
                >
                  <div className="p-4">
                    <div className="flex justify-between">
                      <h5 className="mb-2 font-semibold text-green-800 truncate">فكرة:</h5>
                      <p className="text-sm text-green-600 flex items-center">
                        <CheckCircle className="w-4 h-4 mr-1" />
                      </p>
                    </div>
                    <p className="font-normal text-xl">{item.idea}</p>
                  </div>
                </div>
              ))
            ) : (
  ...
  {ideas.length > 0 ? (
      ...
      >
        <div className="p-5 h-full flex flex-col justify-between">
          <div>
            ...
            <p className="font-semibold text-lg text-gray-800 mt-1">{item.title}</p>

            <p className="font-normal text-lg text-gray-800 mt-1">{item.idea}</p>
          </div>
          ...
          <div className="mt-4">
            ...
            >
              {item.status === "رفض" && <XCircle className="h-4 w-4 mr-1" />}
              {item.status === "انتظار" && <Clock className="h-4 w-4 mr-1" />}
              {item.status === "قبول" && <CheckCircle className="h-4 w-4 mr-1" />}
              {item.status || "انتظار"}
            </p>
    ...
    <p className="text-gray-500 px-5">لا توجد أفكار بعد</p>
        ...
        {teachers.length > 0 && (
          <TeamMembers users={teachers} TeacherId={user.teacherId || null} />
        )}
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center  bg-opacity-50">
    <div className="relative bg-white rounded-lg shadow-lg w-full max-w-md p-6">
      ...
      <div className="p-6 text-center">
        ...
        <h3 className="mb-5 text-lg font-normal text-gray-500">هل أنت متأكد من الحذف؟</h3>
        <button
          type="button"
